...
public class SulphuricCavePiece extends BasePiece {
    ...
    protected void fromNbt(CompoundTag tag) {
        ...
        ventDX = tag.getIntArray("vent_dx");
        ventDZ = tag.getIntArray("vent_dz");
    }
                ...
                for (int y = y1; y <= y2; y++) {
                    ...
                    if (dist < r * r) {
                        ...
                        if (isReplaceable(state)) {
                            chunk.setBlockState(mut, y < waterLevel ? WATER : CAVE_AIR, false);
                        }
                    } else if (dist < r2 * r2) {
                        BlockState state = chunk.getBlockState(mut);
                        if (state.is(CommonBlockTags.END_STONES) || state.is(Blocks.AIR)) {
                            ...
                            if (v > 0.4) {
                                ...
                                chunk.setBlockState(mut, rock, false);
                            }
                        }
                    }
                }
    ...
    private void placeVentColumn(
        ...
        for (int j = 0; j <= dist; j++) {
            chunk.setBlockState(mut, EndBlocks.SULPHURIC_ROCK.stone.defaultBlockState(), false);
        ...
        chunk.setBlockState(mut, EndBlocks.HYDROTHERMAL_VENT.defaultBlockState(), false);
        mut.setY(mut.getY() + 1);
        state = chunk.getBlockState(mut);
        while (state.is(Blocks.WATER)) {
            chunk.setBlockState(mut, EndBlocks.VENT_BUBBLE_COLUMN.defaultBlockState(), false);
    ...
    private void makeShards(ChunkAccess chunk, BlockPos pos, RandomSource random) {
        for (Direction dir : BlocksHelper.DIRECTIONS) {
            ...
            if (random.nextInt(16) == 0 && chunk.getBlockState((side = pos.relative(dir))).is(Blocks.WATER)) {
                ...
                chunk.setBlockState(side, state, false);
            }
        }
    }
}
